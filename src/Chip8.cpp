	...
	: randGen(std::chrono::system_clock::now().time_since_epoch().count())
{
	...
	// FIX: Changed from uint8_t to unsigned short for MSVC compatibility
...
void Chip8::OP_Cxkk()
{
	...
	// FIX: cast from unsigned short to uint8_t since we changed the distribution type
...
void Chip8::OP_Dxyn()
{
	...
	for (unsigned int row = 0; row < height; ++row)
	{
		...
		uint8_t pixelY = yPos + row;

		// Clip the sprite at the bottom edge instead of writing past the buffer.
		if (pixelY >= VIDEO_HEIGHT)
		{
			break;
		}
		...
		{
			uint8_t pixelX = xPos + col;

			// Clip the sprite at the right edge.
			if (pixelX >= VIDEO_WIDTH)
			{
				break;
			}
...
// Decrement the timers. Call this once per 60 Hz frame, not once per
// instruction, so timing is independent of how fast the CPU runs.
